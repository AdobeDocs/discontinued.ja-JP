...
* ユーザーがログインしたり、メールを開いたときなど、個人を識別できる場合は、実装で`setCustomerIDs`関数（またはSDK相当）を呼び出す必要があります。 この要件は、使用される場合、モバイルアプリを含むすべてのプラットフォームに適用されます。 訪問者ID サービス ユーザーガイドの[`setCustomerIDs`](https://experienceleague.adobe.com/docs/id-service/using/id-service-api/methods/setcustomerids.html)を参照してください。
